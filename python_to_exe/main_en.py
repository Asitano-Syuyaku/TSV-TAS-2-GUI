...
class TASConverterApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("TAS Scripts Converter Tool")
        self.resizable(False, False)

        # Input script file (.txt or .tsv)
        tk.Label(self, text="Input Script File (.txt or .tsv):").grid(row=0, column=0, padx=5, pady=5, sticky="e")
        ...
        # Output directory
        tk.Label(self, text="Output Directory:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
        ...
        tk.Label(self, text="Output File Name:").grid(row=2, column=0, padx=5, pady=5, sticky="e")
        ...
        tk.Checkbutton(self, text="Send via FTP (sdmc:/smo/tas/scripts)", variable=self.ftp_var, command=self.toggle_ftp).grid(row=3, column=0, columnspan=3, padx=5, pady=5, sticky="w")
        ...
        tk.Label(self.ftp_frame, text="Port:").grid(row=0, column=2, padx=5, pady=2, sticky="e")
        self.port_entry = tk.Entry(self.ftp_frame)
        self.port_entry.grid(row=0, column=3, padx=5, pady=2)
        tk.Label(self.ftp_frame, text="Username:").grid(row=1, column=0, padx=5, pady=2, sticky="e")
        self.user_entry = tk.Entry(self.ftp_frame)
        self.user_entry.grid(row=1, column=1, padx=5, pady=2)
        tk.Label(self.ftp_frame, text="Password:").grid(row=1, column=2, padx=5, pady=2, sticky="e")
        ...
        # Convert button
        self.convert_btn = tk.Button(self, text="Start Conversion", command=self.start_conversion)
    ...
    def browse_input(self):
        file = filedialog.askopenfilename(filetypes=[("Script Files", "*.txt;*.tsv")])
        if file:
    ...
    def convert(self):
        try:
            ...
            if not input_path or not output_dir or not base_name:
                raise ValueError("Please specify input file, output directory, and output filename.")

            cwd = os.getcwd()
            base_dir = self.get_base_dir()
            self.log(f"Current directory: {cwd}")
            self.log(f"Scripts directory: {base_dir}")

            ext = os.path.splitext(input_path)[1].lower()
            if ext == '.tsv':
                tsv_path = input_path
                self.log("TSV file selected; skipping TXT→TSV conversion.")
            else:
                ...
                self.log("Running: " + " ".join(cmd1))
                res1 = subprocess.run(cmd1, capture_output=True, text=True)
                if res1.returncode != 0:
                    self.log("[Error] nx-tas-to-tsv-tas.py failed")
                    self.log(res1.stderr)
                    raise RuntimeError("nx-tas-to-tsv-tas.py execution failed.")
            ...
            self.log("Running: " + " ".join(cmd2))
            res2 = subprocess.run(cmd2, capture_output=True, text=True)
            if res2.returncode != 0:
                self.log("[Error] tsv-tas.py failed")
                self.log(res2.stderr)
                raise RuntimeError("tsv-tas.py execution failed.")
